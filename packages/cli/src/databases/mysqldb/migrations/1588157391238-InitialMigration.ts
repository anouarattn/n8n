...
export class InitialMigration1588157391238 implements MigrationInterface {
	...
	async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query('CREATE TABLE IF NOT EXISTS`' + tablePrefix + 'delay_entity` (`id` int NOT NULL AUTO_INCREMENT, `executionId` varchar(128) NOT NULL, `endAt` datetime NOT NULL, `delayedNodes` varchar(128) NOT NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB', undefined);
	}
	...
	async down(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query('DROP TABLE `' + tablePrefix + 'delay_entity`', undefined);
